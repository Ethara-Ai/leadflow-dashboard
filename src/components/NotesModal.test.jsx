...
describe('NotesModal', () => {
  ...
  describe('mode toggle', () => {
    ...
    it('should switch to view mode when View Notes is clicked', async () => {
      ...
      expect(
        screen.getByText('Lion enclosure: Male lion showing increased appetite')
      ).toBeInTheDocument();
    });
  ...
  describe('view notes functionality', () => {
    it('should display all notes in view mode', async () => {
      ...
      expect(
        screen.getByText('Penguin habitat: Water filtration maintenance scheduled')
      ).toBeInTheDocument();
      expect(
        screen.getByText('Gorilla section: New enrichment activities working well')
      ).toBeInTheDocument();
    });
  ...
  describe('darkMode override prop', () => {
    it('should use darkMode override when provided (true)', () => {
      const { container } = renderWithTheme(<NotesModal {...defaultProps} darkMode={true} />, {
        darkMode: false,
      });
      const modal = container.querySelector('.bg-slate-800\\/95');
      expect(modal).toBeInTheDocument();
    ...
    it('should use darkMode override when provided (false)', () => {
      const { container } = renderWithTheme(<NotesModal {...defaultProps} darkMode={false} />, {
        darkMode: true,
      });
      const modal = container.querySelector('.bg-white\\/95');
      expect(modal).toBeInTheDocument();
  ...
  describe('edge cases', () => {
    it('should handle notes with very long content', async () => {
      ...
      const notesWithLongContent = [
        { id: 1, content: longContent, timestamp: '1/1/2026, 10:00:00 AM' },
      ];
    ...
    it('should handle notes with special characters', async () => {
      ...
      const notesWithSpecialChars = [
        { id: 1, content: specialContent, timestamp: '1/1/2026, 10:00:00 AM' },
      ];
